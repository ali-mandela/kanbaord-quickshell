...
import {
  groupAndSortDataByUserPriority,
  groupByPriorityAndTitle,
  groupByUserAndTitle,
  groupByPriorityAndPriority,
  groupByStatusAndPriority,
  groupByStatusAndTitle,
} from "./Func/Func";
...
const Text = () => {
  ...
  const [cachedUserStatus, setCachedUserStatus] = useState([
    "User",
    "Priority",
  ]);
  ...
  if (
    cachedUserStatus?.groupBy === "User" &&
    cachedUserStatus?.orderBy === "Priority"
  ) {
    if (cachedResData?.users) {
      groupedData = groupAndSortDataByUserPriority(
        cachedResData?.users,
        cachedResData?.tickets,
      );
    }
  } else if (
    cachedUserStatus?.groupBy === "User" &&
    cachedUserStatus?.orderBy === "Title"
  ) {
    console.log("user and title ");
    if (cachedResData?.tickets && cachedResData?.users) {
      groupedData = groupByUserAndTitle(
        cachedResData?.users,
        cachedResData?.tickets,
      );
    ...
    cachedUserStatus?.groupBy === "Priority" &&
    cachedUserStatus?.orderBy === "Priority"
  ) {
    console.log("priority and priority ");
    if (cachedResData?.tickets && cachedResData?.users) {
      groupedData = groupByPriorityAndPriority(
        cachedResData?.users,
        cachedResData?.tickets,
      );
      console.log(groupedData);
    ...
    if (cachedResData?.tickets && cachedResData?.users) {
      groupedData = groupByPriorityAndTitle(
        cachedResData?.users,
        cachedResData?.tickets,
      );
      console.log(groupedData);
    ...
    cachedUserStatus?.groupBy === "Status" &&
    cachedUserStatus?.orderBy === "Priority"
  ) {
    console.log("Status and priority ");
    if (cachedResData?.tickets && cachedResData?.users) {
      groupedData = groupByStatusAndPriority(
        cachedResData?.users,
        cachedResData?.tickets,
      );
      console.log(groupedData);
    ...
    if (cachedResData?.tickets && cachedResData?.users) {
      groupedData = groupByStatusAndTitle(
        cachedResData?.users,
        cachedResData?.tickets,
      );
      console.log(groupedData);
